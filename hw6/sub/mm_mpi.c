...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        /**
         * Receive results from all other procs
         * 
         */
        MPI_Gather(sendbuf, sendcnt, MPI_INT, recvbuf, recvcnt, MPI_INT, src, MPI_COMM_WORLD);

        execTime += MPI_Wtime();
        printf("Time taken for matrix multiply - mpi: %.2lf", execTime);
